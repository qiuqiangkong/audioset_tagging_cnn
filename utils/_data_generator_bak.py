...
class BalancedMixupSampler(Base):
    def __init__(self, indexes_hdf5_path, black_list_csv, batch_size, 
        ...
        equally sampled from different sound classes. Data with even indexes 
...
class AlternateSampler(Base):
    def __init__(self, indexes_hdf5_path, black_list_csv, batch_size, 
        ...
        Args:
          ...
          random_seed: int
        """
        self.sampler1 = Sampler(indexes_hdf5_path, black_list_csv, batch_size, 
            random_seed)

        self.sampler2 = BalancedMixupSampler(indexes_hdf5_path, black_list_csv, 
            batch_size, start_mix_epoch, random_seed)
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if self.count % 2 == 0:
                ...
                batch_meta = []
                i = 0
                while i < self.sampler2.batch_size // 2:
                    if len(self.sampler2.queue1) == 0:
                        self.sampler2.queue1 = self.sampler2.expand_queue(self.sampler2.queue1)

                    class1_id = self.sampler2.queue1.pop(0)
                    pointer1 = self.sampler2.pointers1_of_classes[class1_id]
                    self.sampler2.pointers1_of_classes[class1_id] += 1
                    index1 = self.sampler2.indexes1_per_class[class1_id][pointer1]
                    
                    # When finish one epoch of a sound class, then shuffle its indexes and reset pointer
                    if self.sampler2.pointers1_of_classes[class1_id] >= self.sampler2.samples_num_per_class[class1_id]:
                        self.sampler2.finished_epochs_per_class[class1_id] += 1
                        self.sampler2.pointers1_of_classes[class1_id] = 0
                        self.sampler2.random_state.shuffle(self.sampler2.indexes1_per_class[class1_id])

                    # If audio in black list then continue
                    if self.sampler2.audio_names[index1] in self.sampler2.black_list_names:
                        continue
                    else:
                        # batch_indexes.append(index1)
                        batch_meta.append({
                            'audio_name': self.sampler2.audio_names[index1], 
                            'hdf5_path': self.sampler2.hdf5_paths[index1], 
                            'index_in_hdf5': self.sampler2.indexes_in_hdf5[index1], 
                            'target': self.sampler2.targets[index1]})
                        i += 1

                    # Append meta if a sound class has been trained after 
                    # start_mix_epoch epochs, otherwise append None.
                    if self.sampler2.finished_epochs_per_class[class1_id] >= self.sampler2.start_mix_epoch:
                        j = 0
                        while j < 1:
                            if len(self.sampler2.queue2) == 0:
                                self.sampler2.queue2 = self.sampler2.expand_queue(self.sampler2.queue2)

                            class2_id = self.sampler2.queue2.pop(0)
                            pointer2 = self.sampler2.pointers2_of_classes[class2_id]
                            self.sampler2.pointers2_of_classes[class2_id] += 1
                            index2 = self.sampler2.indexes2_per_class[class2_id][pointer2]

                            if self.sampler2.pointers2_of_classes[class2_id] >= self.sampler2.samples_num_per_class[class2_id]:
                                self.sampler2.pointers2_of_classes[class2_id] = 0
                                self.sampler2.random_state.shuffle(self.sampler2.indexes2_per_class[class2_id])

                            # If audio in black list then continue
                            if self.sampler2.audio_names[index2] in self.sampler2.black_list_names:
                                continue
                            else:
                                batch_meta.append({
                                    'audio_name': self.sampler2.audio_names[index2], 
                                    'hdf5_path': self.sampler2.hdf5_paths[index2], 
                                    'index_in_hdf5': self.sampler2.indexes_in_hdf5[index2], 
                                    'target': self.sampler2.targets[index2]})
                                j += 1
                    else:
                        batch_meta.append(None)
